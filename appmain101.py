...
import boto3
from reportlab.lib.colors import orange, green, blue, red
...
def add_table_to_story1(dataframe, title,txt, story, styles):
    story.append(Paragraph(title, styles['Heading2']))
...
def split_category_names(category_percentages):
    split_categories = {}
    for category, percentage in category_percentages.items():
        words = category.split()
        if len(words) == 1:
            split_categories[category] = percentage
            continue
        
        # Find the middle space closest to the center
        mid_index = len(category) // 2
        space_index = category[:mid_index].rfind(' ')
        if space_index == -1:
            space_index = category[mid_index:].find(' ') + mid_index
        
        first_part = category[:space_index]
        second_part = category[space_index+1:]
        
        split_categories[first_part + '\n' + second_part] = percentage
    return split_categories

# Determine PSF Sales level
def get_psf_sales_level(sqft):
    if sqft < 1000:
        return "very low level"
    elif 1000 <= sqft <= 1799:
        return "lower than average level"
    elif 1800 <= sqft <= 2499:
...
def clean_text(text):
    ...
    # Remove specified units (gm, kg, rs)
    text = re.sub(r'\b(gm|kg|rs)\b', '', text)
    # Remove extra spaces and any remaining hyphens
    text = re.sub(r'-', ' ', text)
...
def find_most_similar_product1(new_description,df, description_column, category_column):

    df[description_column] = df[description_column].apply(clean_text)
    df[category_column] = df[category_column].apply(clean_text)

    # Create a new column for combined text from description and category
    df['combined_description'] = df[category_column] + " " + df[description_column]
    ...
    new_combined_description = clean_text(new_description)
...
def find_most_similar_product(new_description, new_category, df, description_column, category_column):
    ...
    # Combine the new description and category for the query
    new_combined_description = clean_text(new_description) + " " + clean_text(new_category)
    new_description_vector = tfidf.transform([new_combined_description])   
...
def send_interakt_message(user_number, pdflink, language_code,username):
    url = 'https://api.interakt.ai/v1/public/message/'
    headers = {
...
def convert_to_xlsx1(input_path):
    base, ext = os.path.splitext(input_path)
    if ext.lower() in ['.xls', '.csv']:  # Handle .xls reading but convert to .xlsx
        if ext.lower() == '.csv':
            df = pd.read_csv(input_path)
        else:
            df = pd.read_excel(input_path)
        ...
        df.to_excel(output_path, index=False, engine='openpyxl')
        return output_path
    return input_path  # If it's already .xlsx, just return the original path


# def convert_to_xlsx(file_path):
#     """
#     Ensure that the file path ends with .xlsx. If it doesn't, convert it to .xlsx.
#     """
#     base, ext = os.path.splitext(file_path)
#     if ext.lower() in ['.xls', '.csv']:  # check for .xls and .csv
#         new_path = base + '.xlsx'
#         if ext.lower() == '.csv':
#             df = pd.read_csv(file_path)
#         elif ext.lower() =='.xls':
#             df = pd.read_excel(file_path,engine='xlrd')
#         df.to_excel(new_path, index=False, engine='openpyxl')  # convert to .xlsx
#         print("New path: ",new_path)
#         return new_path  # return new file path
#     elif ext.lower() == '.xlsx':
#         return file_path  # already an .xlsx file
#     else:
#         raise ValueError("Unsupported file format")  # handle other formats or errors
...
# def preprocess_excel(file_path, output_file_path):
#     print(100)
...
#         text_count = df.iloc[i].apply(is_text).sum()
#         if text_count > max_text_count:
#             max_text_count = text_count
#             header_row_index = i
    
#     # Slicing the DataFrame to remove rows above the header row index
...
#     columns_to_drop = missing_values_percentage[missing_values_percentage > 60].index
#     df = df.drop(columns=columns_to_drop)
#     df = df.dropna(subset=[df.columns[0]])
#     df = df.dropna(how='any')
#     header_values = df.columns.tolist()
#     df = df[~df.apply(lambda row: row.tolist() == header_values, axis=1)]
#     missing_values_percentage = df.isna().mean() * 100
...
#             for cell in row:
#                 merged_cells.add((cell.row - 1, cell.column - 1))

#     max_text_count = 0
#     header_row_index = 0
#     for i in range(len(df)):
#         # Skip rows with merged cells
#         if any((i, j) in merged_cells for j in range(len(df.columns))):
#             continue
...
def preprocess_excel(file_path, output_file_path):
    ...
    for i in range(len(df)):
        ...
        # Check if row has text only in one column
        text_count = df.iloc[i].apply(is_text).sum()
        if text_count == 1:
            continue  # Skip this row as a potential header
        
        # Temporarily set this row as header to check column names
        temp_columns = df.iloc[i].astype(str).str.strip()
        if any("Unnamed:" in col for col in temp_columns):
            continue  # Skip this row if it contains "Unnamed:" in any column name
...
def download_file_from_s3(url, local_folder):
...
def safe_divide(numerator, denominator):
    try:
        return ((numerator - denominator) / denominator * 100) if denominator != 0 else None
...
#     month_mapping = request.form.get('month_mapping')  # JSON string mapping filenames to months

#     print("kfid: ",kfid)
#     print("files: ",files)
#     print("month_mapping: ",month_mapping)

#     if not files or not kfid or not month_mapping:
#         return jsonify({'error': 'Missing file, kfid, or month_mapping parameter'}), 400

#     try:
#         month_mapping = json.loads(month_mapping)  # Parse the JSON string to a dictionary
#         print("1")
#     except json.JSONDecodeError:
#         return jsonify({'error': 'Invalid month_mapping JSON format'}), 400
...
#         month = month_mapping.get(file.filename)
#         if not month:
#             return jsonify({'error': f'Month not provided for file {file.filename}'}), 400
...
#             output_file_path = os.path.join(app.config['UPLOAD_FOLDER'], 'preprocessed_' + filename)
#             preprocessed_data = preprocess_excel(file_path, output_file_path)

#             try:
#                 s3.upload_file(output_file_path, S3_BUCKET, 'preprocessed_'+filename)
#                 s3_url = f"https://{S3_BUCKET}.s3.{S3_REGION}.amazonaws.com/preprocessed_{filename}"
...
#             #s3_url = upload_to_s3(file_path, 'your_bucket_name', filename)  # Replace 'your_bucket_name' with your actual S3 bucket name
#             #preprocessed_s3_url = upload_to_s3(output_file_path, 'your_bucket_name', 'preprocessed_' + filename)  # Upload the preprocessed file

#             # Update the database with the link for the respective month
#             # with db.cursor() as cursor:
#             #     update_query = f"UPDATE SalesAnalysisCustomer SET {month}_link = %s WHERE kfid = %s"
#             #     cursor.execute(update_query, (s3_url, kfid))
#             #     db.commit()
...
def upload_excel():
    kfid = request.form.get('kfid')
    files = request.files.getlist('file')
    month_mapping = request.form.get('month_mapping')
    ...
    try:
        month_mapping = json.loads(month_mapping)
    except json.JSONDecodeError:
        return jsonify({'error': 'Invalid month_mapping JSON format'}), 400
    ...
    for file in files:
        ...
        if file:
            ...
            output_file_path = os.path.join(app.config['UPLOAD_FOLDER'], 'preprocessed_' + os.path.basename(file_path))
            print()
            ...
            try:
                s3.upload_file(output_file_path, S3_BUCKET, 'preprocessed_' + os.path.basename(file_path))
                s3_url = f"https://{S3_BUCKET}.s3.{S3_REGION}.amazonaws.com/preprocessed_{os.path.basename(file_path)}"
...
def process_one():
    ...
    optional_filename = request.form.get('optional_filename',None)

    if mandatory_filename:
        mandatory_filename = download_file_from_s3(mandatory_filename, app.config['UPLOAD_FOLDER'])
        print(f"Mandatory file stored at: {mandatory_filename}")

    if optional_filename:
        optional_filename = download_file_from_s3(optional_filename, app.config['UPLOAD_FOLDER'])
        print(f"Optional file stored at: {optional_filename}")
    ...
    if mandatory_filename and not optional_filename:
        ...
        if mandatory_filename:
            ...
            mandatory_category_column = "None"
            ...
            if mandatory_category_column != "None":
                ...
                mandatory_merged = mandatory_agg.merge(mandatory_df[[mandatory_product_description_column]].drop_duplicates(),
                                                    on=mandatory_product_description_column,
                                                    how='left')
            ...
            if mandatory_category_column!= "None":
                for index, row in mandatory_merged.iterrows():
                    ...
                    if new_description in combined_df['Item Name'].values:
                        combined_df.loc[combined_df['Item Name'] == new_description, 'Quantity Sold Second Excel'] = row[mandatory_quantity_sold_column]
                        combined_df.loc[combined_df['Item Name'] == new_description, 'Price Second Excel'] = row[mandatory_price_column]
                    else:
                ...
                for index, row in mandatory_merged.iterrows():
                    ...
                    if new_description in combined_df['Item Name'].values:
                        ...
                        most_similar_product = find_most_similar_product1(new_description, existing_df, 'Product Description', 'CATEGORY')
                        new_row = {
                            'Item Name': new_description,
                            'User Category': None,
                            'Category Predicted': most_similar_product['CATEGORY'].iloc[0] if not most_similar_product.empty else 'Unknown',
            ...
            # combined_df['Quantity Sold First Excel'] = pd.to_numeric(combined_df['Quantity Sold First Excel'], errors='coerce').fillna(0)
            ...
            if mandatory_category_column != "None":
                combined_df['Price First Excel'] = pd.to_numeric(combined_df['Price First Excel'], errors='coerce').fillna(0)
                total_price_first_excel1 = combined_df['Price First Excel'].sum()
                category_price_sums_first11 = combined_df.groupby('Category Predicted')['Price First Excel'].sum()
                category_percentage_first11 = {}
                for category in category_price_sums_first11.index:
                    category_percentage_first11[category] = (category_price_sums_first11[category] / total_price_first_excel1) * 100
                # Sort the dictionary by percentage in descending order to get the top 5
                sorted_categories_top5 = sorted(category_percentage_first11.items(), key=lambda x: x[1], reverse=True)[:10]

                # Sort the dictionary by percentage in ascending order to get the bottom 5
                sorted_categories_bottom5 = sorted(category_percentage_first11.items(), key=lambda x: x[1])[:5]
                ...
                # Sort the dictionary by percentage in descending order to get the top 5
                sorted_categories_top5 = sorted(category_percentage_first1.items(), key=lambda x: x[1], reverse=True)[:10]
                ...
                category_percentage_first11 = {}
                for category in category_price_sums_first11.index:
                    category_percentage_first11[category] =  round((category_price_sums_first11[category] / total_price_first_excel1) * 100, 2)
                # Sort the dictionary by percentage in descending order to get the top 5
                sorted_categories_top5 = sorted(category_percentage_first11.items(), key=lambda x: x[1], reverse=True)[:10]
                

                # Sort the dictionary by percentage in ascending order to get the bottom 5
                sorted_categories_bottom5 = sorted(category_percentage_first11.items(), key=lambda x: x[1])[:10]
                ...
                combined_df['Price First Excel'] = pd.to_numeric(combined_df['Price First Excel'], errors='coerce').fillna(0)
                total_price_first_excel = combined_df['Price First Excel'].sum()
                category_price_sums_first1 = combined_df.groupby('Category Type')['Price First Excel'].sum()
                category_percentage_first1 = {}
                for category in category_price_sums_first1.index:
                    category_percentage_first1[category] = (category_price_sums_first1[category] / total_price_first_excel) * 100
                print('Last Month Analysis')
                for category, percentage in category_percentage_first1.items():
                    print(f"Category Predicted '{category}' makes up {percentage:.2f}% of the Last Month total price.")
            ...
            print("num_of_bills: ",num_of_bills)
            print()
            print()
            ABS = total_item_sold/num_of_bills
            ABS = f"{ABS:.2f}"
            print("Average Basket Size: ",ABS)
            print()
            ...
            destination_food_percentage = category_percentage_first1.get('Destination Food', 0)
            ...
            if destination_food_percentage > 45:
                ...
                destination_food_message = (
            f" Consider strengthening destination categories to bring them to at least 45% contribution. It can even go up to 50% depending upon shopping habits of your core customers."
            " You can do it by:\n"
            ...
            combined_percentage = category_percentage_first1.get('Destination Food', 0) + category_percentage_first1.get('Routine Non Core Food', 0)
            ...
            if combined_percentage > 65:
                ...
                food_categories_message = f"Consider strengthening of your food categories to improve sales and customer loyalty by -\nEnhancing your product range\nEnsuring good display\nEnsuring product availability\nPlanning promotions\nPremiumisation"
            ...
            routine_non_food_percentage = category_percentage_first1.get('Routine Non Food', 0)
            ...
            if mandatory_filename:
                ...
                top_20_itemsSKU['Sales Last Month'] = top_20_itemsSKU['Sales Last Month'].round(2)
                ...
                bottom_20_itemsSKU['Sales Last Month'] = bottom_20_itemsSKU['Sales Last Month'].round(2)
            ...
            if mandatory_filename:
                ...
                top_20_itemsproducts['Sales Last Month'] = top_20_itemsproducts['Sales Last Month'].round(2)
                data104 = [top_20_itemsproducts.columns.tolist()] + top_20_itemsproducts.values.tolist()

                # Get the bottom 20 items
                bottom_20_itemsproducts = df_sorted.tail(20)[['Product Predicted', 'Price First Excel']]
                bottom_20_itemsproducts = bottom_20_itemsproducts.rename(columns={
                    'Price First Excel': 'Sales Last Month',
                })
                bottom_20_itemsproducts['Sales Last Month'] = bottom_20_itemsproducts['Sales Last Month'].round(2)
                data_bottom_20 = [bottom_20_itemsproducts.columns.tolist()] + bottom_20_itemsproducts.values.tolist()


            # Pie Chart Generation
            save_pie_chart(category_percentage_first1, 'last_month_pie_chart.png', 'Last Month Category Percentages')
            ...
            story.append(Paragraph("Part 1: KEY Performance Indicators", styles['Heading2']))
            story.append(Spacer(1, 12))
            story.append(Paragraph("Average Basket Value (ABV): Average Value of each bill", styles['BodyText']))
            story.append(Paragraph(f"(a) Average Basket Value (ABV) = Rs {ABV}", styles['BodyText']))
            story.append(Spacer(1, 12))
            story.append(Paragraph("Average Basket Size (ABS): Average number of items in each bill", styles['BodyText']))
            story.append(Paragraph(f"(b) Average Basket Size (ABS) = {ABS}", styles['BodyText']))
            story.append(Spacer(1, 12))
            story.append(Paragraph("Average Selling Price (ASP): Average price of each item sold", styles['BodyText']))
            story.append(Paragraph(f"(c) Average Selling Price (ASP) = Rs {AIV}", styles['BodyText']))
            story.append(Spacer(1, 12))
            story.append(Paragraph(f"(d) Sales Per Sq.ft. = Rs {sqft}", styles['BodyText']))
            story.append(Spacer(1, 12))
            ...
            story.append(Paragraph("Part 4: Top 10 Contributing Category %", styles['Heading1']))
            ...
            story.append(Spacer(1, 12))
            story.append(Spacer(1, 12))
            story.append(Paragraph("Bottom 10 Contributing Category %", styles['Heading1']))
            ...
            story.append(PageBreak())
            # Section Top SKU
            story.append(Paragraph("Part 5: Top 20 SKU", styles['Heading1']))
            ...
            story.append(table)
            story.append(Spacer(1, 12))
            story.append(Paragraph("Part 5: Bottom 20 SKU", styles['Heading1']))
            story.append(Paragraph("Insights: This part of the report shows you bottom 20 on SKU level", styles['BodyText']))
            story.append(Spacer(1, 12))
            table = Table(data199)
            table.setStyle(TableStyle([
                ('BACKGROUND', (0,0), (-1,0), colors.grey),
            ...
            # Section Top Product
            story.append(Paragraph("Part 6A: Top 20 Products", styles['Heading1']))
            ...
            print()
            print()
            response = send_interakt_message(mobile_number, presigned_url, language_code, username)
            print()
            print('Response: ',response)
            print()
        ...
        mandatory_category_column = "None"  # New
        ...
        print("mandatory_product_description_column: ",mandatory_product_description_column)
        print("mandatory_quantity_sold_column: ",mandatory_quantity_sold_column)
        print("mandatory_price_column: ",mandatory_price_column)
        print("mandatory_category_column: ",mandatory_category_column)
        print("optional_product_description_column: ",optional_product_description_column)
        print("optional_quantity_sold_column: ",optional_quantity_sold_column)
        print("optional_price_column: ",optional_price_column)
        print("optional_category_column: ",optional_category_column)
        print("num_of_bills: ",num_of_bills)
        print("no_of_bills: ",no_of_bills)
        print("store_szie: ",store_szie)
        ...
        if mandatory_filename:
            ...
            mandatory_df[mandatory_product_description_column] = mandatory_df[mandatory_product_description_column].str.strip()
            
            # Convert columns to numeric, forcing errors to NaN and then filling NaN with 0
            mandatory_df[mandatory_quantity_sold_column] = pd.to_numeric(mandatory_df[mandatory_quantity_sold_column], errors='coerce').fillna(0)
            mandatory_df[mandatory_price_column] = pd.to_numeric(mandatory_df[mandatory_price_column], errors='coerce').fillna(0)
            ...
            if mandatory_category_column != "None":
                ...
                print()
                print()
                print("result_categories1: ", result_categories1)
                print()
                print()
                ...
                # Sort each column based on the percentage values
                sorted_categories = []
                for col in zip(*result_categories1):
                    header = col[0]
                    items = sorted(col[1:], key=parse_percentage, reverse=True)
                    sorted_categories.append([header] + items)
                ...
                for col in sorted_categories:
                    ...
                    adjusted_sorted_categories.append(col)
                ...
                print()
                print()
                print("sorted_result_categories1: ", sorted_result_categories)
                print()
                print()
                ...
                category_price_sums_first1 = combined_df.groupby('Category Predicted')['Price First Excel'].sum()
                category_percentage_first1 = {}
                ...
                for category in category_types:
                    filtered_df = combined_df[combined_df['Category Type'] == category]
                    unique_user_categories = filtered_df['Category Predicted'].unique().tolist()
                    category_sales = []
                ...
                # Prepare data for table, ensuring equal row count across all columns
                sorted_result_categories = [list(column) for column in zip(*adjusted_sorted_categories)]
                print()
                print()
                print("sorted_result_categories1: ", sorted_result_categories)
                print()
            ...
            psf_sales_level = get_psf_sales_level(sqft)
        ...
        if optional_filename:
            ...
            optional_df[optional_product_description_column] = optional_df[optional_product_description_column].str.strip()

            # Convert columns to numeric, forcing errors to NaN and then filling NaN with 0
            optional_df[optional_quantity_sold_column] = pd.to_numeric(optional_df[optional_quantity_sold_column], errors='coerce').fillna(0)
            optional_df[optional_price_column] = pd.to_numeric(optional_df[optional_price_column], errors='coerce').fillna(0)
            ...
            if optional_category_column != "None":
                ...
                mandatory_merged1 = optional_agg.merge(optional_df[[optional_product_description_column]].drop_duplicates(),
                                                        on=optional_product_description_column,
                                                        how='left')
            new_rows = []
            if optional_category_column!= "None":
            ...
            growthABV = ((ABV - ABV1)/ABV1)*100
            growthABS = ((ABS - ABS1)/ABS1)*100
            growthAIV = ((AIV - AIV1)/AIV1)*100
            growthsqft = ((sqft - sqft1)/sqft1)*100
            
            growthpercentage = ((GwothAPICuurent - GwothAPIPrevious)/GwothAPIPrevious)*100
        ...
        if optional_category_column != "None":
            ...
            top_20_data = [['Category Type', 'Value Growth', 'Quantity Growth']] + top_20[['Category Type', 'Value Growth', 'Quantity Growth']].values.tolist()
            bottom_20_data = [['Category Type', 'Value Growth', 'Quantity Growth']] + bottom_20[['Category Type', 'Value Growth', 'Quantity Growth']].values.tolist()
        ...
        if mandatory_filename and optional_filename:
            ...
            if 0 <= quantity_percentage100_overall <= 1:
                ...
                overallvolume = (f"Volume growth is at {quantity_percentage100_overall:.2f}% which is good.")
            elif 10 <= quantity_percentage100_overall < 20:
                ...
                overallvolume = (f"Volume degrowth is {quantity_percentage100_overall:.2f}%  is small but shouldn't be ignored..")
            ...
            if 0 <= price_percentage100_overall < 1:
                ...
                overallprice = (f"Value growth is at {price_percentage100_overall:.2f}% which is good")
            elif 10 <= price_percentage100_overall < 20:
                ...
                overallprice = (f"Value degrowth is {price_percentage100_overall:.2f}% is small but shouldn't be ignored.")
        ...
        if mandatory_filename and optional_filename:
            ...
            if total_quantity_first > 0 and total_price_first > 0:
                ...
                if 0 <= quantity_percentage100 < 1:
                    ...
                    destinationvolume = (f"Volume growth is at {quantity_percentage100:.2f}% which is good.")
                elif 10 <= quantity_percentage100 < 20:
                    ...
                    destinationvolume = (f"Volume degrowth is {quantity_percentage100:.2f}%  is small but shouldn't be ignored..")
                ...
                if 0 <= price_percentage100 < 1:
                    ...
                    destinationprice = (f"Value growth is at {price_percentage100:.2f}% which is good.")
                elif 10 <= price_percentage100 < 20:
                    ...
                    destinationprice = (f"Value degrowth is {price_percentage100:.2f}%  is small but shouldn't be ignored..")
        ...
        if mandatory_filename and optional_filename:
            ...
            if total_quantity_first > 0 and total_price_first > 0:
                ...
                if 0 <= quantity_sold_percentage_change < 1:
                    ...
                    foddvolume = (f"Volume growth is at {quantity_sold_percentage_change:.2f}% which is good.")
                elif 10 <= quantity_sold_percentage_change < 20:
                    ...
                    foddvolume = (f"Volume degrowth is {quantity_sold_percentage_change:.2f}%  is small but shouldn't be ignored..")
                ...
                if 0 <= price_percentage_change < 1:
                    ...
                    foodprice = (f"Value growth is at {price_percentage_change:.2f}% which is good.")
                elif 10 <= price_percentage_change < 20:
                    ...
                    foodprice = (f"Value degrowth is {price_percentage_change:.2f}%  is small but shouldn't be ignored..")
        ...
        if mandatory_filename and optional_filename:
            ...
            if total_quantity_first > 0 and total_price_first > 0:
                ...
                if 0 <= quantity_percentage102 < 1:
                    ...
                    routinevolume = (f"Volume growth is at {quantity_percentage102:.2f}% which is good.")
                elif 10 <= quantity_percentage102 < 20:
                    ...
                    routinevolume = (f"Volume degrowth is {quantity_percentage102:.2f}%  is small but shouldn't be ignored..")
                ...
                if 0 <= price_percentage102 < 1:
                    ...
                    routineprice = (f"Value growth is at {price_percentage102:.2f}% which is good.")
                elif 10 <= price_percentage102 < 20:
                    ...
                    routineprice = (f"Value degrowth is {price_percentage102:.2f}%  is small but shouldn't be ignored..")
        ...
        if No_of_Families_Shopping_Estimate > Average_Monthly_Basket_Estimate:
            # salt_msg = f"No. of Families shopping is {No_of_Salt_Packets:.0f}.\n Average Monthly Basket Estimate is Rs{No_of_Families_Shopping_Estimate:.0f}.\n Total Sales is Rs{Total_Sales:.0f}."
            salt_msg = (f"No. of Families shopping is <b>{No_of_Salt_Packets:,.0f}</b><br /><br />"
                f"Average Monthly Basket Estimate is <b>Rs {No_of_Families_Shopping_Estimate:,.0f}</b><br /><br />"
                f"Total Sales is <b>Rs {Total_Sales:,.0f}</b>")
            comparison_message = f"The No of Families Shopping Estimate is greater than the Average Monthly Basket Estimate which is Rs{No_of_Families_Shopping_Estimate:.0f}"
        elif No_of_Families_Shopping_Estimate < Average_Monthly_Basket_Estimate:
            # salt_msg = f"No. of Families shopping is {No_of_Salt_Packets:.0f}.\n Average Monthly Basket Estimate is Rs{No_of_Families_Shopping_Estimate:.0f}.\n Total Sales is Rs{Total_Sales:.0f}."
            salt_msg = (f"No. of Families shopping is <b>{No_of_Salt_Packets:,.0f}</b><br /><br />"
        ...
        if mandatory_filename:
            ...
            SKU_msg = (f"Total SKU is <b>{total_sku}</b>.<br /><br />"
            f"Total Sales is <b>Rs{total_sales:,.2f}</b>.<br /><br />"
            f"<b>{num_top_80_percent_items}</b> SKU's contribute to 80% of sales which is <b>{percentageofsku:.2f}%</b> of total SKU's.")
        ...
        if optional_category_column != "None":
            ...
            aggregated_data108['Value_Growth'] = aggregated_data108.apply(lambda x: safe_divide(x['Price First Excel'], x['Price Second Excel']), axis=1)
            aggregated_data108['Quantity_growth'] = aggregated_data108.apply(lambda x: safe_divide(x['Quantity Sold First Excel'], x['Quantity Sold Second Excel']), axis=1)

            # Filter positive and negative Value Growth
            positive_value_growth = aggregated_data108[aggregated_data108['Value_Growth'] > 0]
            negative_value_growth = aggregated_data108[aggregated_data108['Value_Growth'] < 0]

            # Get top 20 and bottom 20 categories for Value Growth
            top20Cat = positive_value_growth.nlargest(20, 'Value_Growth')   
            bottom20Cat = negative_value_growth.nsmallest(20, 'Value_Growth')
            ...
            top20Cat = top20Cat.rename(columns={
                'Category Predicted': 'Category',
            })
            bottom20Cat = bottom20Cat.rename(columns={
                'Category Predicted': 'Category',
            })
            ...
            aggregated_data108 = combined_df.groupby('Category Predicted').agg({
                'Quantity Sold First Excel': 'sum',
                'Quantity Sold Second Excel': 'sum',  
                "Price First Excel": "sum",
                "Price Second Excel": "sum"
            ...
            # Select only the relevant columns for the table
            columns = ['Category Predicted', 'Value Growth', 'Quantity Growth']

            # Calculate value and volume growth without adding '%' yet
            aggregated_data108['Value Growth'] = aggregated_data108.apply(lambda x: safe_divide(x['Price First Excel'], x['Price Second Excel']), axis=1)
            aggregated_data108['Quantity Growth'] = aggregated_data108.apply(lambda x: safe_divide(x['Quantity Sold First Excel'], x['Quantity Sold Second Excel']), axis=1)
            ...
            # Filter positive and negative Value Growth
            positive_value_growth = aggregated_data108[aggregated_data108['Value Growth'] > 0]
            negative_value_growth = aggregated_data108[aggregated_data108['Value Growth'] < 0]

            # Get top 20 and bottom 20 categories for Value Growth
            top20Cat = positive_value_growth.nlargest(20, 'Value Growth')   
            bottom20Cat = negative_value_growth.nsmallest(20, 'Value Growth')

            # Format growth fields as percentage strings rounded to two decimal places
            aggregated_data108['Value Growth'] = aggregated_data108['Value Growth'].round(2).astype('str') + '%'
            aggregated_data108['Quantity Growth'] = aggregated_data108['Quantity Growth'].round(2).astype('str') + '%'
            ...
            # Format growth fields as percentage strings rounded to two decimal places for both top and bottom categories
            for df in [top20Cat, bottom20Cat]:
                df['Value Growth'] = df['Value Growth'].apply(lambda x: '{:.2f}%'.format(x) if pd.notnull(x) else 'nan')
                df['Quantity Growth'] = df['Quantity Growth'].apply(lambda x: '{:.2f}%'.format(x) if pd.notnull(x) else 'nan')
        ...
        if mandatory_filename and optional_filename:
            ...
            Product_msg = (f"Total Product is <b>{total_product}</b>.<br /><br />"
            f"Total Sales is <b>Rs {total_sales:,.2f}</b>.<br /><br />"
            f"<b>{num_top_80_percent_items1}</b> Products contribute to 80% of sales which is <b>{percentageofsku1:.2f}%</b> of total Products.")
        ...
        if mandatory_filename and optional_filename:
            ...
            filtered_df = aggregated_data10.dropna(subset=['Price ASP First', 'Price ASP Second'])
            
            filtered_data10 = filtered_df[(filtered_df['Price ASP First'] != 0) & (filtered_df['Price ASP Second'] != 0)]
            print()
            print()
            print(filtered_data10)
            print()
            print()
            filtered_data10['ASP Growth (%)'] = round(((filtered_data10['Price ASP First'] - filtered_data10['Price ASP Second']) / filtered_data10['Price ASP Second'] * 100).astype(int),2)
            
            # Get the top 25 ASP growth
            top_25_asp = filtered_data10.sort_values(by='ASP Growth (%)', ascending=False).head(20)
            top_25_asp['ASP Growth (%)'] = round(top_25_asp['ASP Growth (%)'],2)
            top_25_asp['ASP Growth (%)'] = top_25_asp['ASP Growth (%)'].astype(str) + '%'
            top_25_asp = top_25_asp[['Product Predicted', 'Price ASP First', 'Price ASP Second', 'ASP Growth (%)']]
            top_25_asp = top_25_asp.rename(columns={
                'Product Predicted': 'Product',
                'Price ASP First': 'ASP Last Month',
                'Price ASP Second': 'ASP Previous Month'
            })
            top_25_asp['ASP Last Month'] = round(top_25_asp['ASP Last Month'],2)
            top_25_asp['ASP Previous Month'] = round(top_25_asp['ASP Previous Month'],2)
            ...
            bottom_25_asp = filtered_data10.sort_values(by='ASP Growth (%)', ascending=True).head(20)
            bottom_25_asp['ASP Growth (%)'] = round(bottom_25_asp['ASP Growth (%)'],2)
            bottom_25_asp['ASP Growth (%)'] = bottom_25_asp['ASP Growth (%)'].astype(str) + '%'
            bottom_25_asp = bottom_25_asp[['Product Predicted', 'Price ASP First', 'Price ASP Second', 'ASP Growth (%)']]
            bottom_25_asp = bottom_25_asp.rename(columns={
                'Product Predicted': 'Product',
                'Price ASP First': 'ASP Last Month',
                'Price ASP Second': 'ASP Previous Month'
            })
            bottom_25_asp['ASP Last Month'] = round(bottom_25_asp['ASP Last Month'],2)
            bottom_25_asp['ASP Previous Month'] = round(bottom_25_asp['ASP Previous Month'],2)
        ...
        #     })
            ...
            top_20_growth_data_Product_display['Quantity Sold in Last Month'] = top_20_growth_data_Product_display['Quantity Sold in Last Month'].round(2).astype(int)
            top_20_growth_data_Product_display['Quantity Sold in Previous Month'] = top_20_growth_data_Product_display['Quantity Sold in Previous Month'].round(2).astype(int)
            bottom_20_de_growth_data_Product_display['Quantity Sold in Last Month'] = bottom_20_de_growth_data_Product_display['Quantity Sold in Last Month'].round(2).astype(int)
            bottom_20_de_growth_data_Product_display['Quantity Sold in Previous Month'] = bottom_20_de_growth_data_Product_display['Quantity Sold in Previous Month'].round(2).astype(int)
        ...
        if mandatory_filename and optional_filename:
            ...
            top_20_growth_data['Quantity Sold First Excel'] = top_20_growth_data['Quantity Sold First Excel'].round(2)
            top_20_growth_data['Quantity Sold Second Excel'] = top_20_growth_data['Quantity Sold Second Excel'].round(2)
            ...
            bottom_20_degrowth_data['Quantity Sold First Excel'] = bottom_20_degrowth_data['Quantity Sold First Excel'].round(2)
            bottom_20_degrowth_data['Quantity Sold Second Excel'] = bottom_20_degrowth_data['Quantity Sold Second Excel'].round(2)
        ...
        if mandatory_filename and optional_filename:
            save_pie_chart(split_category_names(category_percentage_first), 'last_month_pie_chart.png', 'Last Month Business Contribution')
            save_pie_chart(split_category_names(category_percentage_second), 'previous_month_pie_chart.png', 'Previous Month Business Contribution')
        else:
            save_pie_chart(split_category_names(category_percentage_first), 'last_month_pie_chart.png', 'Last Month Business Contribution')
        ...
        if mandatory_filename and optional_filename:
            ...
            styles.add(ParagraphStyle(name='CenteredBoldHeading2', parent=styles['Heading2'], alignment=TA_CENTER, fontName='Helvetica-Bold'))
            styles.add(ParagraphStyle(name='CenteredHeading1', parent=styles['Heading1'], alignment=TA_CENTER))
            styles.add(ParagraphStyle(name='OrangeHeading1', parent=styles['Heading1'], textColor=red))
            styles.add(ParagraphStyle(name='BoldHeading4', parent=styles['Heading4'], spaceAfter=0))
            green_text_style = ParagraphStyle(name='GreenText', parent=styles['BodyText'], textColor=green, fontName='Helvetica-Bold')
            styles.add(green_text_style)

            # Header Section
            image_path = '/home/ubuntu/SalesAnalysisTool/KiranaFriends-logo-without-background.png'  # Update this with the correct path to your image
            img = Image(image_path, width=2*inch, height=2*inch)  # Adjust size as needed
            img.hAlign = 'CENTER'
            story.append(img)
            story.append(Spacer(1, 12))
            story.append(Paragraph("Insight & Action Report", styles['CenteredBoldHeading2']))
            # story.setStrokeColor(colors.black)
            # storysetlinewidth(1.0)
            story.append(Paragraph("Month: June 2024", styles['CenteredBoldHeading2']))
            story.append(Spacer(1, 12))
            story.append(Spacer(1, 12))
            # i want this text in between the page but not in bold
            story.append(Paragraph("This Insight & Action Report provides you useful information about your business at various levels like Business Groups, Categories Products and SKUs. It makes important observations that is essential for you to know and provides you recommended actions for you to consider to improve your sales performance and profits.", styles['BodyText']))
            story.append(Spacer(1, 12))
            story.append(Spacer(1, 12))
            story.append(Paragraph("Disclaimer", styles['CenteredBoldHeading2']))
            story.append(Paragraph("The analysis has been done using computers and may have certain inaccuracies. While we are continually improving the tool, we recommend that before you take any corrective action, please verify the information and evaluate the recommendations. You should only take action when you believe it is the right thing to do for you.", styles['BodyText']))
            story.append(PageBreak())

            # Section 1: 
            # story.append(Paragraph("Part 1:-", styles['Heading2']))
            story.append(Paragraph("Part 1:- Key Performance Indicators", styles['OrangeHeading1']))
            story.append(Paragraph("These are important indicators for your store's sales performance", styles['BodyText']))
            story.append(Spacer(1, 12))
            story.append(Paragraph("<b>A. Average Basket Value (ABV)</b> - Average Value of each bill", styles['BodyText']))
            story.append(Paragraph(f"Average Basket Value = Rs <b>{ABV:,.2f}</b>", styles['BodyText']))
            story.append(Paragraph(f"Growth over previous month is <b>{growthABV:.2f}%</b>", styles['BodyText']))
            story.append(Spacer(1, 12))
            story.append(Paragraph("<b>B. Average Basket Size (ABS)</b> - Average number of items in each bill", styles['BodyText']))
            story.append(Paragraph(f"Average Basket Size = <b>{ABS:,.2f}</b> units", styles['BodyText']))
            story.append(Paragraph(f"Growth over previous month is <b>{growthABS:.2f}%</b>", styles['BodyText']))
            story.append(Spacer(1, 12))
            story.append(Paragraph("<b>C. Average Selling price (ASP)</b> - Average price of each item sold", styles['BodyText']))
            story.append(Paragraph(f"Average Selling price = Rs <b>{AIV:,.2f}</b>", styles['BodyText']))
            story.append(Paragraph(f"Growth over previous month is <b>{growthAIV:.2f}%</b>", styles['BodyText']))
            story.append(Spacer(1, 12))
            story.append(Paragraph(f"<b>D. Sales Per Sq. Ft (SPSF)</b> - Average sales per square feet of your store", styles['BodyText']))
            story.append(Paragraph(f"Sales Per Sq.ft = <b>Rs {sqft:,.0f}</b>", styles['BodyText']))
            story.append(Paragraph(f"Growth over previous month is <b>{growthsqft:.2f}%</b>", styles['BodyText']))
            story.append(Paragraph(f"<b>Insight: Your sales PSF is {psf_sales_level}</b>", styles['BodyText']))
            story.append(Spacer(1, 12))
            #story.append(Paragraph(f"(e) Growth over previous month = {GrowthAPI:.0f}%", styles['BodyText']))
            #story.append(Spacer(1, 12))

            # story.append(Paragraph("Part 2", styles['Heading2']))
            story.append(Paragraph("Part 2:- Family Basket", styles['OrangeHeading1']))
            story.append(Paragraph("This part of the report shows the estimated number of families shopping with you this month and their basket assessment, based on your salt packet sales which is a good indicator of family shoppers.", styles['BodyText']))
            # story.append(Paragraph("This part of the report shows you the estimates of numbers of families who have shopped with you in the given month and their shopping basket assessment.", styles['BodyText']))
            # story.append(Paragraph("Number of packets of salt you sell is a good indicator of number of families shopping with you. This working is based on your salt packet sales.", styles['BodyText']))
            story.append(Spacer(1, 12))
            story.append(Paragraph("<b>INSIGHTS</b>", styles['BodyText']))
            story.append(Paragraph(f"{salt_msg}", styles['BodyText']))
            story.append(Spacer(1, 12))
            ...
            story.append(Paragraph("3. Target improving ABS and ASP", styles['BodyText']))
            story.append(Spacer(1, 12))
            story.append(Paragraph("Your sales increase comes from –", styles['BodyText']))          
            story.append(Paragraph("– More families shopping with you", styles['BodyText']))                                                                    
            story.append(Paragraph("– Every family spending more with you", styles['BodyText']))                                                                
            story.append(Paragraph("Reach out to more families through marketing, retain the families coming to you and encourage them to shop more through improved product range, promotions and premiumization.", styles['BodyText']))                                                                           
            story.append(PageBreak())


            # story.append(Paragraph("Part 3", styles['Heading2']))
            story.append(Paragraph("Part 3:- Business Group Performance", styles['OrangeHeading1']))
            story.append(Paragraph("This part of the analysis shows you the performance of your various business groups that consist of similar categories in the way customers see them. It considers the benchmark participation of these business groups and provides you insights and actions required,", styles['BodyText']))
            last_month_image = Image('last_month_pie_chart.png')
            last_month_image._restrictSize(400, 400)
            story.append(last_month_image)
            story.append(Spacer(1, 12))
            story.append(Paragraph("Business Groups & their meaning", styles['BodyText']))
            story.append(Spacer(1, 12))
            story.append(Paragraph("Destination Food Categories – These are core food categories that include Dry Grocery, Oil & Ghee, Masalas, Salt & Sugar, Dry Fruits and Upwas Items. These categories are the reason customers come to you and remain with you. This is the foundation of a Supermarket business.", styles['BodyText']))
            story.append(Spacer(1, 12))                                                                                                                         
            story.append(Paragraph("Routine Non-core Food – These are all other categories of food including Tea, Coffee, Chocolates, Biscuits, Namkeens, Diary, frozen food etc. which are not covered by Destination Food categories.", styles['BodyText']))                                                      
            story.append(Spacer(1, 12))
            story.append(Paragraph("Routine non Food Categories – These are all the non-food categories that form part of monthly basket of your customers. It includes Laundry, Bath, Dental, Cleaning, Hair, Cosmetics etc.", styles['BodyText']))                                                                
            story.append(Spacer(1, 12))
            story.append(Paragraph("The above three groups are essential for completeness of a family’s monthly shopping basket and typically consist of 28 categories in all", styles['BodyText']))                                                                                                                
            story.append(Spacer(1, 12))
            story.append(Paragraph("Non Routine Categories – Some Supermarkets may have certain categories that are typically not part of monthly basket. For example, utensils, kitchen tools, toys, stationery etc.", styles['BodyText']))
            story.append(Spacer(1, 12))
            # if mandatory_category_column != "None":
            story.append(Paragraph("Unique User Categories by Category Type", styles['Heading2']))
            story.append(Spacer(1, 12))
            table_data = list((sorted_result_categories))
            table = Table(table_data)
            table.setStyle(TableStyle([
            ...
            # story.append(Paragraph("Part 4", styles['Heading2']))
            story.append(Paragraph("Part 4:- Business Group Growth", styles['OrangeHeading1']))
            ...
            story.append(Paragraph(f"Volume Growth: <b>{quantity_percentage100_overall:.2f}%</b>", styles['BodyText']))
            story.append(Paragraph(f"   Note: {overallvolume}", styles['BodyText']))
            story.append(Paragraph(f"Value Growth: <b>{price_percentage100_overall:.2f}%</b>", styles['BodyText']))
            ...
            story.append(Paragraph(f"Volume Growth: <b>{quantity_percentage100:.2f}%</b>", styles['BodyText']))
            story.append(Paragraph(f"   Note: {destinationvolume}", styles['BodyText']))
            story.append(Paragraph(f"Value Growth: <b>{price_percentage100:.2f}%</b>", styles['BodyText']))
            story.append(Paragraph(f"   Note: {destinationprice}", styles['BodyText']))
            story.append(Spacer(1, 12))
            story.append(Paragraph("C. All Food Categories", styles['Heading3']))
            story.append(Paragraph(f"Volume Growth: <b>{quantity_sold_percentage_change:.2f}%</b>", styles['BodyText']))
            story.append(Paragraph(f"   Note: {foddvolume}", styles['BodyText']))
            story.append(Paragraph(f"Value Growth: <b>{price_percentage_change:.2f}%</b>", styles['BodyText']))
            ...
            story.append(Paragraph(f"Volume Growth: <b>{quantity_percentage102:.2f}%</b>", styles['BodyText']))
            story.append(Paragraph(f"   Note: {routinevolume}", styles['BodyText']))
            story.append(Paragraph(f"Value Growth: <b>{price_percentage102:.2f}%</b>", styles['BodyText']))
            story.append(Paragraph(f"   Note: {routineprice}", styles['BodyText']))
            story.append(Spacer(1, 12))
            story.append(PageBreak())


            # story.append(Paragraph("Part 5", styles['Heading2']))
            story.append(Paragraph("Part 5:- Category Growth", styles['OrangeHeading1']))
            ...
            story.append(Paragraph(f"Top Growing Categories by Value", styles['Heading3']))
            ...
            story.append(Paragraph("Least Growing Categories by Value", styles['Heading3']))
            ...
            # story.append(Paragraph("Part 6", styles['Heading2']))
            story.append(Paragraph("Part 6:- Product Sales Performance", styles['OrangeHeading1']))
            story.append(Paragraph("This section shows your business growth at the product level. It evaluates sales performance based on the product itself, not considering the brand, variant, or size. This helps you see the sales volumes of key products your customers buy.", styles['BodyText']))
            # story.append(Paragraph("This part of the report shows you business growth at product level", styles['BodyText']))
            # story.append(Paragraph("For evaluating sales performance at product level, product is considered irrespective of the brand, variant or size. This helps you understand sales volumes of the key products your customers buy. Of course, Brand, Variant and size does come later.", styles['BodyText']))
            ...
            add_table_to_story1(top_25_asp,"Top 20 ASP","These products have shown increase in Average Selling Price", story, styles)
            story.append(Paragraph("<b>RECOMMENDATION</b>", styles['BodyText']))
            ...
            add_table_to_story1(bottom_25_asp,"Bottom 20 ASP","These products have shown decrease in Average Selling Price", story, styles)
            story.append(Paragraph("<b>RECOMMENDATION</b>", styles['BodyText']))
            story.append(Paragraph("Analyse the causes", styles['BodyText']))
            story.append(Paragraph("Is it because of the change in product mix", styles['BodyText']))
            ...
            # story.append(Paragraph("Part 7", styles['Heading2']))
            story.append(Paragraph("Part 7:- SKU Sales Growth", styles['OrangeHeading1']))
            story.append(Paragraph("This section shows your business growth at the Stock Keeping Unit (SKU) level. An SKU is a unique item you stock and sell. For example, a 100g Parle G is a different SKU from a 50g Parle G of the same brand and type.", styles['BodyText']))
            #story.append(Paragraph("This part of the report shows you business growth at SKU level", styles['BodyText']))
            #story.append(Paragraph("An SKU (Stock Keeping Unit) is a unique item that your stock and sell. For example a tooth paste in 100 gms will be a different SKU from a tooth paste of 50 gms which is of the same brand and type.", styles['BodyText']))
            ...
            story.append(Spacer(1, 12))
            add_table_to_story(less_then_5_sku_filtered, "SKUs with less than 10 unit sales in the month", story, styles)
            # story.append(Paragraph("<b>Insight</b>", styles['BodyText']))
            # story.append(Paragraph("Slow selling SKUs", styles['BodyText']))
            story.append(Spacer(1, 12))
            story.append(Paragraph("<b>RECOMMENDATION</b>", styles['BodyText']))
