...
def send_interakt_message(user_number, message_name, language_code):
    url = 'https://api.interakt.ai/v1/public/message/'
    headers = {
        'Authorization': 'Bearer your_actual_interakt_token',  # Update this
...
def GenerateSalesReport():
    #if 'mandaotry_filename':
    if False:
        print('-----------------------------------------------Im Here in one month Analysis Report-----------------------')
        ...
        #account_sid = 'AC28136d6d8cb892a692b8477a7e151128'
        #auth_token = '64c8fb859a55f2e44664018db5231e51'
        #client = Client(account_sid, auth_token)
        
        #message = client.messages.create(
         #       from_='+918010442222',  # Twilio WhatsApp number
         #       body='Your sales report will generate in 2 to 10 min.',
         #       to='+919892485682'  # Recipient's WhatsApp number with country code
         #   )
        # Start virtual display
        #virtual_display = Display(visible=0, size=(800, 600))
        #virtual_display.start()
        #pywhatkit.sendwhatmsg("+919892485682","Test Msg",18,30)
        ...
        #print("optional_filename: ",optional_filename)
        ...
        if mandatory_filename:
            ...
            user_number = request.form.get('mobile_number', None)
            template_name = "sales_analysis"
            language_code = "hi"

            response = send_interakt_message(user_number, template_name, language_code)
            ...
            for category in category_price_sums_first1.index:
                print("category_price_sums_first1[category]:",category_price_sums_first1[category])
                print("total_price_first_excel:",total_price_first_excel)
            ...
            print()
            print()
            print("Category Price of User Category is:",category_price_sums_first1)
            print()
            print()

            print("Category Percentage of User Category is:",category_percentage_first1)
            print()
            print()
            ...
            top5_category_names = [category[0].strip() for category in sorted_categories_top5]
            top5_percentage_values = [round(category[1],2) for category in sorted_categories_top5]
            ...
            bottom5_category_names = [category[0].strip() for category in sorted_categories_bottom5]
            bottom5_percentage_values = [round(category[1],2) for category in sorted_categories_bottom5]
            ...
            if No_of_Salt_Packets > 0:
                ...
                #No_of_Families_Shopping_Estimate = f"{No_of_Families_Shopping_Estimate:.2f}"
            else:
                No_of_Families_Shopping_Estimate = 0
            # Fixed value for Average Monthly Basket Estimate
            Average_Monthly_Basket_Estimate = 4000.00  # Rs
            # Step 4: Compare and determine if it's greater or less
            if No_of_Families_Shopping_Estimate > Average_Monthly_Basket_Estimate:
                salt_msg = f"No. of family shopping is {No_of_Salt_Packets}. Average Monthly Basket Estimate is Rs{No_of_Families_Shopping_Estimate:.2f}. Total Sales is Rs{Total_Sales:.2f}."
            ...
            if mandatory_filename:
                ...
                bottom20_product_names = [item[0] for item in data199[1:]]
                bottom20_product_price = [round(item[1],2) for item in data199[1:]]
            ...
            if mandatory_filename:
                ...
                product_names = [item[0] for item in data104[1:]]
                product_price = [round(item[1],2) for item in data104[1:]]
            ...
            story.append(Paragraph(f"(b) Average Basket Size (ABS) = {ABS} units", styles['BodyText']))
            story.append(Spacer(1, 12))
            ...
            story.append(Paragraph("Insight: Shows you categories percentage", styles['BodyText']))
            story.append(Spacer(1, 12))
            ...
            story.append(table)
            story.append(Spacer(1, 12))
            story.append(Paragraph("Bottom 20 SKU", styles['Heading1']))
            ...
            return jsonify({
                ...
                "total_sales": f"{total_price_first_excel:.2f}",
                "no_of_family_shoping": No_of_Salt_Packets,
                "average_monthly_basket_estimate": f"{No_of_Families_Shopping_Estimate:.2f}",
                "Business_group_performance_percentage": category_percentage_first,
                "top5_category_name":top5_category_names,
                "top5_category_percentage":top5_percentage_values,
                "bottom5_category_names":bottom5_category_names,
                "bottom5_category_percentage":bottom5_percentage_values,
                "top20_product_name":product_names,
                "top20_product_price":product_price,
                "bottom20_product_name":bottom20_product_names,
                "bottom20_product_price":bottom20_product_price,
                "presigned_url": presigned_url,
                "store_size":store_szie
                }
                })
    else:
        # Here im just Printing what i have uploaded
        #print("Session contents New:")
        #for key in session:
        #    print(f"{key}: {session[key]}")
        ##############################
        print("----------------------------------------2 Months Report---------------------------------------")
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        ...
        if optional_filename:
            ...
            # Group by item description and sum the quantities and prices
            optional_agg = optional_df.groupby(optional_product_description_column).agg({optional_quantity_sold_column: 'sum',optional_price_column: 'sum'}).reset_index()
        ...
        print()
        print()
        print("Unique User Categories by Category Type",transposed_data)
        print()
        print()
        ...
        if mandatory_filename and optional_filename:
            ...
            story.append(Spacer(1, 12))
            story.append(Paragraph("Sales Analysis Insight & Action Report", styles['CenteredBoldHeading2']))
            story.append(Paragraph("Month: April 2024", styles['CenteredBoldHeading2']))
            story.append(Spacer(1, 12))
            story.append(Paragraph("This Sales Analysis Tool provides you useful information about your business at various levels like Business Groups, Categories Products and SKUs. It makes important observations that is essential for you to know and provides you recommended actions for you to consider to improve your sales performance and profits.", styles['BodyText']))
            story.append(Spacer(1, 12))
            story.append(Paragraph("Disclaimer – The analysis has been done using computers and may have certain inaccuracies. While we are continually improving the tool, we recommend that before you take any corrective action, please verify the information and evaluate the recommendations. You should only take action when you believe it is the right thing to do for you.", styles['BodyText']))
            ...
            story.append(Spacer(1, 12))
            add_table_to_story(less_then_5_sku_filtered, "SKUs with less than 10 unit sales in the month <br /> Insight: Slow selling SKUs <br /><br /> Recommendation: <br /> Consider eliminating them from the range <br />  Introduce new products in its place ", story, styles)
            story.append(Spacer(1, 12))
